...
public class ESTestController {
    ...
    @ApiOperation(value = "es测试创建索引接口sasdaw我还是发哈司法是否收到回复", notes = "es测试创建索引接口")
...
//        TermQueryBuilder termQueryBuilder = QueryBuilders.termQuery("name.keyword", name);//精准查询
        PrefixQueryBuilder prefixQueryBuilder = QueryBuilders.prefixQuery("name", "张");//前缀查询
...
//        FuzzyQueryBuilder fuzzyQueryBuilder = QueryBuilders.fuzzyQuery("name", "三");//模糊查询
        ...
        boolQueryBuilder.must(rangeQueryBuilder).must(prefixQueryBuilder);//and or  查询
        //boolQueryBuilder.should(prefixQueryBuilder);//and or  查询
    ...
    @ApiOperation(value = "es测试聚合查询地址接口", notes = "es测试聚合查询地址接口")
    ...
    public ResponseBean testESFindAddress() {
        ...
        try {
            ...
            for (Terms.Bucket bucket : buckets) {
                long docCount = bucket.getDocCount();//个数
                String keyAsNumber = bucket.getKeyAsString();//年龄
                System.err.println(keyAsNumber + "岁的有" + docCount + "个");
    ...
    @ApiOperation(value = "es测试聚合 什么鬼查询地址接口", notes = "es测试聚合查询地址接口")
    ...
    //限定了查询的队伍名称
    // searchSourceBuilder.query(QueryBuilders.termQuery("address","火箭")).size(100);
...

    {
        ...
        return new ResponseBean(200, "查询成功", playerList);
      }
    }catch (IOException e) {
        e.printStackTrace();
    }
